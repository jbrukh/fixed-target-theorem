...
\title{The Fixed-Target Theorem}
\author{Jake Brukhman\\\texttt{jake@coinfund.io}
\and GPT 5.6 Sol}
\date{}

\hypersetup{
  pdftitle={The Fixed-Target Theorem},
  pdfauthor={Jake Brukhman and GPT 5.6 Sol}
}
...
proved by Kaneko and Locke~\cite{KanekoLocke2001}.  We give a short
fixed-target counting proof that the conjecture holds for every oriented graph
of order $n$ and minimum outdegree $\delta$ satisfying $n\le 2\delta+2$.
\end{abstract}
...
vertices $u,v$, at most one of $u\to v$ and $v\to u$ is present.
...
call $v$ the \emph{source vertex} and $x$ the \emph{target vertex}.

A vertex $v$ is a \emph{Seymour vertex} if
\[
                         |\NTwo(v)|\ge|\NOne(v)|.
\]
Seymour's Second Neighborhood Conjecture says that every finite oriented graph
...
\]
This is the fixed-target observation: every source missing $x$, together with
...
\begin{proof}
By (1), every arc whose source lies in $P(x)$ has its target in $T(x)$.
There are at least $\delta|P(x)|$ such arcs.  At most
$\binom{|P(x)|}{2}$ have both endpoints in $P(x)$, because $G$ is oriented,
and at most
$|P(x)|\bigl(|T(x)|-|P(x)|\bigr)$ go from $P(x)$ to the rest of the trap.
Consequently, if $P(x)\ne\varnothing$, then
\[
 \delta|P(x)|
 \le \binom{|P(x)|}{2}
    +|P(x)|\bigl(\delta+q(x)-|P(x)|\bigr).
\]
Dividing by $|P(x)|$ and rearranging gives
$|P(x)|\le2q(x)-1$.  This is impossible when $q(x)=0$, so then
$P(x)=\varnothing$.
\end{proof}
...
\]
This explains the double count: $x\in U(v)$ and $v\in P(x)$ describe the
