...
direction to generalized stars~\cite{Ghazal2012}.  We give a short
...
Two remarks locate the content of Theorem~\ref{thm:fixed-target}.  First,
every oriented graph satisfies $n\ge2\delta+1$, and equality is attained only
when every pair of vertices is adjacent and every outdegree equals $\delta$,
that is, when $G$ is a regular tournament.  The case $n=2\delta+1$ is
therefore contained in Fisher's theorem~\cite{Fisher1996}, although the proof
below is independent and short.  The new content is the case $n=2\delta+2$,
where up to $n/2$ edges may be missing and no structure is imposed on them,
in contrast to~\cite{FidlerYuster2007,Ghazal2012}; conversely, those results
impose no lower bound on the minimum outdegree, so neither result contains
the other.  Second, the theorem shows that any counterexample to the
conjecture must satisfy $n\ge2\delta+3$: counterexamples, if any exist, can
be made close to regular tournaments in the sense
of~\cite{GuoKangZwaneveld2026}, but they cannot occur at the two densest
orders an oriented graph of minimum outdegree $\delta$ can have.
...
\begin{equation}\label{eq:demand}
 |U(v)|=n-1-\delta-|\NTwo(v)|\ge n-2\delta=c.
\end{equation}
...
If $c=1$, then \eqref{eq:ledger} gives
\[
                           0<Q\le\ell-n\le0,
\]
a contradiction.  If $c=2$, then \eqref{eq:lower}--\eqref{eq:ledger} give
\[
                           I<2Q\le2\ell\le I,
\]
...
$c=n-2\delta=3$, the ledger \eqref{eq:ledger} gives only $Q\le\ell+n$, while
\eqref{eq:lower} gives $I\ge3\ell$; since $\ell\le n$, the comparison
